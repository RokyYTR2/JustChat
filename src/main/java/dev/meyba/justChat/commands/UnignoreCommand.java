...
public class UnignoreCommand implements CommandExecutor, TabCompleter {
    ...
    public UnignoreCommand(JustChat plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 1) {
            player.sendMessage(plugin.getChatUtils().formatMessage("&cUsage: /unignore <player>"));
            return true;
        }
        ...
        if (!plugin.getIgnoreManager().isIgnoring(player.getUniqueId(), targetPlayer.getUniqueId())) {
            player.sendMessage(plugin.getChatUtils().formatMessage(
                    plugin.getChatUtils().getMessage("not-ignoring").replace("%player%", targetPlayer.getName())));
            return true;
        }

        plugin.getIgnoreManager().unignorePlayer(player.getUniqueId(), targetPlayer.getUniqueId());
        player.sendMessage(plugin.getChatUtils().formatMessage(
                plugin.getChatUtils().getMessage("unignore-success").replace("%player%", targetPlayer.getName())));
